<x-filament-panels::page>
    <div class="space-y-6" dir="rtl">
        ...
        <div class="flex items-center gap-3 flex-wrap bg-white dark:bg-gray-800 rounded-xl p-4 shadow">
            ...
            <input type="date" value="{{ $selectedDate }}" max="{{ today()->toDateString() }}"
            ...
            @else
                <span class="text-xs bg-yellow-100 text-yellow-700 px-2 py-1 rounded-full">⚠️ آمار گذشته — فقط
                    نمایش</span>
            @endif
        </div>
        ...
        <div class="rounded-xl overflow-hidden shadow border dark:border-gray-700">
            <div
                class="bg-gray-800 text-white px-4 py-2 font-bold text-center text-sm print:bg-white print:text-black print:border-b">
                ...
                <table class="w-full text-xs text-center">
                    ...
                    <tbody>
                        @php
                            $rows = [
                                'organizational' => [
                                    'label' => 'موجودی سازمانی',
                                    'class' => 'bg-blue-50 dark:bg-blue-900/30 font-bold',
                                ],
                                'actual' => [
                                    'label' => 'موجودی واقعی',
                                    'class' => 'bg-gray-50 dark:bg-gray-800 font-bold',
                                ],
                                'present' => [
                                    'label' => 'حاضر',
                                    'class' => 'bg-green-50 dark:bg-green-900/30 font-bold text-green-700',
                                ],
                                ...
                                'arrested' => [
                                    'label' => 'بازداشت',
                                    'class' => 'bg-red-50 dark:bg-red-900/20 text-red-600',
                                ],
                                'course' => ['label' => 'دوره', 'class' => ''],
                            ];
                            $rowClasses = [
                                'organizational' => 'bg-blue-50 font-bold',
                                'actual' => 'bg-gray-50 font-bold',
                                'present' => 'bg-green-50 font-bold text-green-700',
                                'mission' => '',
                                'leave' => '',
                                'medical' => '',
                                'absent' => 'bg-red-50 text-red-600',
                                'arrested' => 'bg-red-50 text-red-600',
                                'course' => '',
                            ];
                        ...
                        @foreach ($rows as $key => $row)
                           <tr class="{{ $rowClasses[$key] ?? '' }}">
                                <td class="px-3 py-2 text-right border dark:border-gray-700 font-medium">
                                    {{ $row['label'] }}</td>
                ...
                @if (count($absentList[$status]) > 0)
                    <div class="rounded-lg border dark:border-gray-700 overflow-hidden text-sm">
                        <div class="bg-gray-100 dark:bg-gray-700 px-3 py-2 font-bold flex justify-between">
                            <span>{{ $label }}</span>
                            <span
                                class="bg-white dark:bg-gray-800 text-xs px-2 rounded-full">{{ count($absentList[$status]) }}</span>
                        </div>
            ...
            <div class="rounded-xl border dark:border-gray-700 overflow-hidden">
                ...
                <div class="px-4 py-2 flex gap-2 flex-wrap border-b dark:border-gray-700">
                    @foreach (['' => 'همه', 'officer_a' => 'افسر الف', 'officer_b' => 'افسر ب', 'vazife_officer' => 'افسر وظیفه', 'nco' => 'د.پایور', 'vazife_nco' => 'د.وظیفه', 'soldier' => 'سرباز'] as $val => $lbl)
                        <button wire:click="$set('rankFilter', '{{ $val }}')"
                    ...
                    <table class="w-full text-sm">
                        ...
                        <tbody class="divide-y dark:divide-gray-700">
                            @foreach ($personnel as $p)
                                ...
                                <tr
                                    class="{{ $currentStatus !== 'present' ? 'bg-yellow-50 dark:bg-yellow-900/10' : '' }}">
                                    ...
                                    <td class="px-4 py-2 text-center">
                                        ...
                                        <select
                                            ...
                                            class="text-xs border rounded px-2 py-1 bg-gray-800 border-gray-600 {{ $statusClasses[$currentStatus] ?? '' }}">
                                            <option value="present" {{ $currentStatus === 'present' ? 'selected' : '' }}>✅
                                                حاضر</option>
                                            <option value="mission" {{ $currentStatus === 'mission' ? 'selected' : '' }}>📋
                                                مأمور</option>
                                            <option value="leave" {{ $currentStatus === 'leave' ? 'selected' : '' }}>🏖️
                                                مرخصی</option>
                                            <option value="medical" {{ $currentStatus === 'medical' ? 'selected' : '' }}>🏥
                                                بهداری</option>
                                            <option value="absent" {{ $currentStatus === 'absent' ? 'selected' : '' }}>⚠️
                                                غیبت</option>
                                            <option value="arrested" {{ $currentStatus === 'arrested' ? 'selected' : '' }}>
                                                🔒 بازداشت</option>
                                            <option value="course" {{ $currentStatus === 'course' ? 'selected' : '' }}>📚
                                                دوره</option>
                                        </select>
                                    </td>
                                    <td class="px-4 py-2">
